test_that("nfer correctly handles cycle_one", {
  # skip as we have to set --full and this isn't supported yet from R
  skip("--full option not yet supported in R")
  spec   <- nfer::load(system.file("extdata", "cycle_one.nfer", package = "nfer"))
  input  <- nfer::read(system.file("extdata", "cycle_one.events", package = "nfer"))
  ...
  expect_equal(row$start, 0)
  expect_equal(row$end, 0)
  expect_equal(row$v, "4")
  row <- result[5,]
  expect_equal(row$name, "A")
  expect_equal(row$start, 0)
  expect_equal(row$end, 0)
  expect_equal(row$v, "5")
})
